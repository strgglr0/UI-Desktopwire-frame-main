...
const MUTATING_ACTIONS = new Set([
  ...
  'mgr-approve-report',
  'mgr-return-report',
  'sa-final-approve-report',
  'generate-clinic-report',
  'delete-generated-report',
  'reset-demo-data',
  'save-ptstaff-app-url',
  'clear-ptstaff-app-url'
]);
...
function bindAppActions() {
  document.getElementById('app').addEventListener('click', async (e) => {
    ...
    const shouldLock = MUTATING_ACTIONS.has(action);
    if (shouldLock) setButtonBusy(target, true);
    ...
    try {
      ...
      if (action === 'view-month-log' || action === 'view-year-log') {
        ...
        if (!staffId) {
          showToast('Staff ID not found for this attendance row.', 'error');
          return;
        }
        ...
        if (!window.confirm(`Delete staff account ${id}? This cannot be undone.`)) return;
        ...
        if (!window.confirm(`Delete user account ${id}? This cannot be undone.`)) return;
        ...
        if (!reportName || !reportType || !department || !periodStart || !periodEnd) {
          showToast('Please complete all required report details.', 'error');
          return;
        }
        ...
        if (!window.confirm('Reset demo data to defaults? This will replace current in-app records.')) return;
        ...
        if (!value) {
          showToast('Please enter a PT Staff app URL.', 'error');
          return;
        }
        if (!/^https?:\/\//i.test(value)) {
          showToast('Please enter a valid URL that starts with http:// or https://', 'error');
          return;
        }
        localStorage.setItem(PTSTAFF_APP_URL_KEY, value);
        showToast('PT Staff app URL saved. PT Staff logins will now redirect there.', 'success');
      } else if (action === 'clear-ptstaff-app-url') {
        localStorage.removeItem(PTSTAFF_APP_URL_KEY);
        const input = document.getElementById('ptstaff-app-url-input');
        if (input) input.value = '';
        showToast('PT Staff app URL cleared. PT Staff logins will use local placeholder workspace.', 'info');
      }

      if (result?.status === 'notified') {
        showToast('Change applied and Super Admin has been notified.', 'info');
      ...
      if (shouldLock) setButtonBusy(target, false);
    }
  });
  ...
  document.getElementById('app').addEventListener('submit', async (e) => {
    ...
    try {
      if (e.target.id === 'register-staff-form') {
        ...
        await refreshData();
        renderScreen(activeScreen);
        showToast('Medical staff account registered.', 'success');
        return;
      }
      ...
      if (e.target.id === 'create-user-form') {
        ...
        await refreshData();
        renderScreen(activeScreen);
        showToast('Login user created successfully.', 'success');
        return;
      }
      ...
      if (e.target.id === 'create-ptstaff-user-form') {
        ...
        await refreshData();
        renderScreen(activeScreen);
        showToast('PT Staff user account created successfully.', 'success');
      }
    } catch (err) {
...
function filterAttendanceByView(attendance, viewMode) {
  ...
  const todayContext = getTodayAttendanceContext(rows);
  const effectiveToday = todayContext.effectiveDate;
  ...
  return rows.filter((record) => {
    ...
    if (viewMode === 'today') return !!effectiveToday && dateVal === effectiveToday;
...
function getTodayAttendanceContext(attendance) {
  const rows = attendance || [];
  ...
  const validDates = rows
    ...
    .filter((d) => /^\d{4}-\d{2}-\d{2}$/.test(d))
    .sort();
  const latestDate = validDates.length ? validDates[validDates.length - 1] : '';
  const hasCurrentDateRows = rows.some((record) => String(record.date || '').slice(0, 10) === todayStr);
  return {
    todayDate: todayStr,
    latestDate,
    hasCurrentDateRows,
    effectiveDate: hasCurrentDateRows ? todayStr : latestDate,
    isFallback: !hasCurrentDateRows && !!latestDate,
    hasAny: !!latestDate
  };
}
...
function hrAttendanceHtml(data) {
  ...
  const context = getTodayAttendanceContext(data.attendance || []);
  const rows = renderAttendanceTableRows(todayRows, true);
  const contextText = !context.hasAny
    ? 'No attendance data available yet.'
    : context.isFallback
      ? `Showing latest available attendance day: <strong>${context.effectiveDate}</strong> (no records found for today).`
      : `Showing current-day attendance: <strong>${context.effectiveDate}</strong>.`;
  return `<article class="card"><h3>HR Attendance View</h3><p class="muted">${contextText} To manage full monthly or yearly logs, use <strong>Month Log</strong> or <strong>Year Log</strong> beside each staff name.</p><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Staff ID</th><th>Name</th><th>Department</th><th>Date</th><th>Time In</th><th>Time Out</th><th>Hours</th><th>OT</th><th>On-Call</th><th>Leave</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div></article>`;
}
...
function managerAttendanceHtml(data) {
  ...
  const contextText = !context.hasAny
    ...
    : context.isFallback
      ? `Showing latest available attendance day: <strong>${context.effectiveDate}</strong> (no records found for today).`
      : `Showing current-day attendance: <strong>${context.effectiveDate}</strong>.`;
  return `<article class="card"><h3>Display Staff Attendance</h3><p class="muted">${contextText} Use <strong>Month Log</strong> / <strong>Year Log</strong> beside each staff name for full logs.</p><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Staff ID</th><th>Name</th><th>Department</th><th>Date</th><th>Time In</th><th>Time Out</th><th>Hours</th><th>OT</th><th>Status</th></tr></thead><tbody>${rows}</tbody></table></div></article>`;
}
...
function bindModal() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    if (mode === 'reason') {
      ...
      if (required && !value) {
        showToast('Please provide a reason/comment.', 'error');
        return;
      }
      closeModal(value);
      return;
...
function updateSyncLabel() {
  const syncEl = document.getElementById('data-sync-label');
  if (!syncEl) return;
  if (!lastDataSyncAt) {
    syncEl.textContent = 'Data not synced yet';
    return;
  }
  syncEl.textContent = `Last sync: ${lastDataSyncAt.toLocaleTimeString()}`;
}
